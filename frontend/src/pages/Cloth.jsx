      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex flex-col lg:flex-row gap-16">
          ...
          <aside className="w-full lg:w-64 flex-shrink-0 space-y-10">
            <div>
              <h2 className="text-sm font-black uppercase tracking-[0.2em] text-black mb-8">Filters</h2>
              
              {/* Size Filter */}
              <div className="mb-10">
                <h3 className="text-[11px] font-black uppercase tracking-widest mb-6 flex justify-between items-center">
                  Size
                </h3>
                <div className="flex flex-wrap gap-2">
                  {['XS', 'S', 'M', 'L', 'XL', '2XL'].map(size => (
                    <button 
                      ...
                      className={`w-10 h-10 border text-[10px] font-bold flex items-center justify-center transition-all ${
                        currentSize === size ? "bg-black text-white border-black" : "border-black text-black hover:bg-black hover:text-white"
                      }`}
                    >
                      {size}
                    </button>
              ...
              <div className="border-t border-gray-100 py-6">
                <button 
                  ...
                  className="flex items-center justify-between w-full text-[11px] font-black uppercase tracking-widest text-black"
                >
                  <span>Availability</span>
                  <svg className={`h-4 w-4 transition-transform ${openSections.availability ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                {openSections.availability && (
                  <div className="mt-6 space-y-4">
                    {[
                      ...
                      <button
                        key={opt.value}
                        onClick={() => handleFilterChange("availability", currentAvailability === opt.value ? "" : opt.value)}
                        className={`block w-full text-left text-[11px] uppercase tracking-widest font-bold transition-colors ${
                          currentAvailability === opt.value ? "text-black" : "text-gray-400 hover:text-black"
                        }`}
                      >
                        {opt.label}
                      </button>
              ...
              <div className="border-t border-gray-100 py-6">
                ...
                >
                  ...
                  <svg className={`h-4 w-4 transition-transform ${openSections.price ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                {openSections.price && (
                  <div className="mt-6 flex items-center gap-4">
                    <div className="relative flex-1">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] text-gray-400 font-bold">Rs</span>
                      <input 
                        type="number" 
                        placeholder="0"
                        value={minPriceInput}
                        onChange={(e) => setMinPriceInput(e.target.value)}
                        onBlur={() => handleFilterChange("minPrice", minPriceInput)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            handleFilterChange("minPrice", minPriceInput);
                          }
                        }}
                        className="w-full border border-black/10 py-3 pl-8 pr-2 text-[10px] font-bold outline-none focus:border-black transition-all"
                      />
                    </div>
                    <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold">to</span>
                    <div className="relative flex-1">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] text-gray-400 font-bold">Rs</span>
                      <input 
                        type="number" 
                        ...
                        onBlur={() => handleFilterChange("maxPrice", maxPriceInput)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            handleFilterChange("maxPrice", maxPriceInput);
                          }
